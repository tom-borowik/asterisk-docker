...
for i in `find $1 -type f`; do sox $i -r 8000 -c 1 $2/`basename $i | sed -e 's/.wav/.gsm/'` gain -n -0.5; done
...
# Uppercase the extension for FS...
for m in `find $3 -type f -iname "*.ulaw"`; do mv $m $3`basename $m | sed -s 's/.ulaw/.PCMU/'`; done
for n in `find $3 -type f -iname "*.alaw"`; do mv $n $3`basename $n | sed -s 's/.alaw/.PCMA/'`; done
for o in `find $3 -type f -iname "*.g729"`; do mv $o $3`basename $o | sed -s 's/.g729/.G729/'`; done